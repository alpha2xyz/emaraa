...
export async function registerRoutes(httpServer: Server, app: Express): Promise<Server> {
  ...
  // TEMPORARY — PDF generation proof-of-concept for the e-signature build (2026-09-15).
  // Remove once proven on a Vercel preview deployment; see ~/.claude/plans/fancy-wiggling-blanket.md §1.
  app.get("/api/_debug/pdf-poc", async (_req, res) => {
    try {
...
<style>
  ...
  .en { direction: ltr; font-family: Arial, sans-serif; margin-top: 24px; }
  h1 { color: #065B98; }
</style>
</head>
<body>
  <h1>إثبات توليد PDF من الخادم</h1>
  <p>هذا اختبار لعرض النص العربي بشكل صحيح مع اتجاه الكتابة من اليمين لليسار، بما في ذلك الحروف المتصلة مثل: مؤسسة، إدارة، عِمارة، والأرقام ١٢٣٤٥.</p>
  <div class="en">
    <h2>Server-side PDF generation proof-of-concept</h2>
    <p>This tests correct rendering of English text alongside Arabic, generated by Chromium on Vercel.</p>
  </div>
</body>
</html>`;
      ...
      res.status(500).json({ error: "pdf_poc_failed", message: e?.message, stack: e?.stack });
    }
  });
