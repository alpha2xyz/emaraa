...
**PDF proposal lock rule (added 2026-06-02, UX changed 2026-09-07):** the owner still cannot open a provider's PDF until they **accept** the offer — that gate is unchanged and still enforced server-side (`GET /api/owner/offers/:requestId` nulls out `offer_file_url` for any offer whose status is not `accepted`; `PATCH /api/offers/:id/status` never returns it for a rejected offer either). What changed is the *trigger*: there is no longer a separate "Accept" button followed by a separate "View File" click. A single "View Full Quotation" button (visible while `status === "pending"`) opens the same confirm dialog the old Accept button used, and confirming both accepts the offer (auto-rejecting every other offer on the request, unchanged) **and** opens the now-unlocked PDF immediately — the accept endpoint's response now includes `offer_file_url` so the client doesn't need a second round-trip. Rationale: real owner feedback (2026-09-07, see `memory/project_emaraa_feedback_ibrahim_sakina9.md`) showed owners wouldn't accept an offer they couldn't first read in full, so the two-step flow was quietly killing conversions rather than protecting against them.
